...
static bool HandleMouseDevice(const AzFramework::InputChannel& inputChannel, Rml::Context* ctx)
{
    ...
    if (channelId == AzFramework::InputDeviceMouse::Movement::X || channelId ==
...
void TuInput::Shutdown()
{
    AzFramework::InputChannelEventListener::Disconnect();
}

void TuInput::OnActivate(Rml::TextInputContext* ctx)
{
    m_activeTxtContext = ctx;
    AzFramework::InputTextNotificationBus::Handler::BusConnect();
}

void TuInput::OnDeactivate(Rml::TextInputContext* ctx)
{
    m_activeTxtContext = nullptr;
    AzFramework::InputTextNotificationBus::Handler::BusDisconnect();
}

void TuInput::OnDestroy(Rml::TextInputContext* ctx)
{
    m_activeTxtContext = nullptr;
    AzFramework::InputTextNotificationBus::Handler::BusDisconnect();
}

bool TuInput::OnInputChannelEventFiltered(const AzFramework::InputChannel& inputChannel)
{
    const AzFramework::InputChannelId& channelId = inputChannel.GetInputChannelId();
...
void TuInput::OnInputTextEvent(const AZStd::string& text, bool& consumed)
{
    if (text.empty())
    {
        return;
